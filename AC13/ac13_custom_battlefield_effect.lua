    ...
    return cardChoiceSelectorEffect(
        {
            ...
            effectFirst = createCardEffect(wisdom_of_the_wolf(), loc(currentPlayer(), skillsPloc)).seq(
                createCardEffect(wolf_strength_skill(), loc(oppPlayer(), skillsPloc))
            ).seq(nuUndoEffect()).seq(drawCardsWithAnimation(3)),
            effectSecond = createCardEffect(wolf_strength_skill(), loc(currentPlayer(), skillsPloc)).seq(
                createCardEffect(wisdom_of_the_wolf(), loc(oppPlayer(), skillsPloc))
            ).seq(nuUndoEffect()).seq(drawCardsWithAnimation(3)),
...
function healthMod(value)
    return ifInt(
        selectLoc(currentSkillsLoc).where(isCardName("strength_of_the_wolf")).count().eq(1),
        multiply(toIntExpression(value), const(2)),
        toIntExpression(value)
    )
end
...
function goldMod(value)
    return ifInt(
        selectLoc(currentSkillsLoc).where(isCardName("cunning_of_the_wolf")).count().eq(1),
        multiply(toIntExpression(value), const(2)),
        toIntExpression(value)
    )
end
